...
class FakeTokenCounter:
    """TokenCounter de test : approxime 1 token = 1 mot (rapide, deterministe)."""
...
class TestFixedSizeChunker:
    def test_chunks_multi_source_text(self) -> None:
        chunker = FixedSizeChunker(
            token_counter=FakeTokenCounter(), chunk_size_tokens=15, overlap_tokens=3
        )
        chunks = chunker.chunk([("corpus.md", MULTI_SOURCE_TEXT)])
        _assert_common_invariants(chunks, {"file01", "file02"})

    def test_no_marker_falls_back_to_given_source(self) -> None:
        chunker = FixedSizeChunker(
            token_counter=FakeTokenCounter(), chunk_size_tokens=5, overlap_tokens=1
        )
    ...
    def test_rejects_invalid_overlap(self) -> None:
        with pytest.raises(ValueError):
            FixedSizeChunker(token_counter=FakeTokenCounter(), chunk_size_tokens=10, overlap_tokens=10)
...
class TestRecursiveChunker:
    ...
    def test_small_text_is_single_chunk(self) -> None:
        chunker = RecursiveChunker(token_counter=FakeTokenCounter(), chunk_size_tokens=100)
...
class TestMarkdownChunker:
    def test_groups_by_header_and_keeps_source(self) -> None:
        chunker = MarkdownChunker(token_counter=FakeTokenCounter())
        chunks = chunker.chunk([("corpus.md", MULTI_SOURCE_TEXT)])
        _assert_common_invariants(chunks, {"file01", "file02"})
...
class TestChunkerFactory:
    def test_creates_fixed_from_config(self) -> None:
    ...
    def test_creates_recursive_from_config(self) -> None:
        chunker = create_chunker(
    ...
    def test_creates_markdown_from_config(self) -> None:
        chunker = create_chunker({"strategy": "markdown"}, FakeTokenCounter())
        assert isinstance(chunker, MarkdownChunker)

    def test_unknown_strategy_raises(self) -> None:
        with pytest.raises(ValueError):
            create_chunker({"strategy": "does-not-exist"}, FakeTokenCounter())

    def test_missing_strategy_key_raises(self) -> None:
        with pytest.raises(KeyError):
            create_chunker({}, FakeTokenCounter())
